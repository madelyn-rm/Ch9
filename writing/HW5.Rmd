...
summer <- homicides_dates %>% 
  ...
  count() %>% 
  mutate(total_date = paste(months, years, sep = ", ")) %>% 
...
ggplot() +
  geom_bar(summer, mapping = aes(x = reported_date), fill = "lightgray", color = "gray50") +
  geom_bar(winter, mapping = aes(x = reported_date), bins = 100, fill = "lightblue", color = "gray50") +
